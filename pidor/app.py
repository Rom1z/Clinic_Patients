...
def create_table():
    ...
    cursor.execute('''
        CREATE TABLE IF NOT EXISTS users (
            ...
            age INT NOT NULL,
            phone VARCHAR(20)  # Добавлено поле для номера телефона
        )
    ''')
    conn.commit()
    conn.close()
...
def add_user():
    name = entry_name.get()
    age = entry_age.get()
    phone = entry_phone.get()  # Получение номера телефона
    ...
    if name and age and phone:
        try:
            ...
            conn = create_connection()
            cursor = conn.cursor()
            # Включаем phone в запрос вставки
            cursor.execute("INSERT INTO users (name, age, phone) VALUES (%s, %s, %s)", (name, age, phone))
            conn.commit()
            conn.close()
            messagebox.showinfo("Информация", "Пациент добавлен")
            entry_name.delete(0, ttk.END)
            entry_age.delete(0, ttk.END)
            entry_phone.delete(0, ttk.END)  # Очищаем поле номера телефона
            display_users()
